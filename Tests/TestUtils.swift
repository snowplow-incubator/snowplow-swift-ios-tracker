...
class TestUtils: XCTestCase {
    ...
    func testUrlEncodingDictionary() {
        ...
        XCTAssertEqual(Utilities.urlEncode(twoKeys), "a=b&c=d")

        let intValues = [
            "a": NSNumber(value: -5),
            "c": NSNumber(value: 3)
        ]
        XCTAssertEqual(Utilities.urlEncode(intValues), "a=-5&c=3")

        let boolValues = [
            "a": NSNumber(value: false),
            "c": NSNumber(value: true)
        ]
        XCTAssertEqual(Utilities.urlEncode(boolValues), "a=0&c=1")

        let encodedValues = [
            "a": " " as NSObject,
            "c": "=" as NSObject
        ]
        XCTAssertEqual(Utilities.urlEncode(encodedValues), "a=%20&c=%3D")
    }
