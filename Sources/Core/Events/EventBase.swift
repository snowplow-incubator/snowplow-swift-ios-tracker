...
// TODO: change to protocol once all translated to Swift
/// The inspectable properties of the event used to generate contexts.
@objc(SPInspectableEvent)
public class InspectableEvent: NSObject {
    /// The schema of the event
    @objc public var schema: String? { get { abort() } }
    /// The name of the event
    @objc public var eventName: String? { get { abort() } }
    /// The payload of the event
    @objc public var payload: [String : NSObject] { get { abort() } }
    /// The tracker state at the time the event was sent.
    @objc public var state: TrackerStateSnapshot? { get { abort() } }
    ...
    @objc public func addPayloadValues(_ payload: [String : NSObject]) -> Bool { abort() }
}
...
public class Event: NSObject {
    ...
    @objc public func beginProcessing(withTracker tracker: Tracker) {
    }

    /// Hook method called just after the event processing in order to execute special operations.
    /// @note Internal use only - Don't use in production, it can change without notice.
    @objc public func endProcessing(withTracker tracker: Tracker) {
    }
}
